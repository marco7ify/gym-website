...
- Used fresh isolated origins for every cache and visible checkpoint; the saved `/Users/tony/Downloads/gym-planner-export-all-layouts-2026-08-11.json` was imported through the rendered Import control without displaying its settings or any secret. The final cache contract is `gltf-runtime.js?v=36` and exactly 11 ordered classic scripts: model-assets v4, wall-features v3, garage-doors v1, app v85, equipment-models v2, garage-door-3d v1, view3d v39, panels v73, layout v86, events v82, and render v70. Strict browser RED evidence preceded each production URL change: the initial contract reported runtime v33 instead of v34 and the old 9-script chain; the placement-boundary regression reported app v84 instead of v85/runtime v34 instead of v35; the dense-frame regression reported view3d v38 instead of v39/runtime v35 instead of v36.
...
- Visual inspection in 3D and Walkthrough confirmed one matte-black traditional four-section, 16-panel door; jambs, bottom seal, handle, vertical/curved/overhead tracks, torsion bar/springs, opener rail/arm/motor, and the 2-foot black header were all visible. Both 1 ft 11 in returns remained, and the panel face did not visibly clip the Gazelle or Combo against the wall. The left mirror remained level across the Stair/dumbbell platform transition; slats and three frame LEDs covered the Ice Barrel run and stopped before the missing left-wall span; the right mirror and top cardio LED rendered at their relocated walls. The walkthrough minimap showed the thick amber garage segment while the standard entry retained its green hinged leaf.
