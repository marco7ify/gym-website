...
function layoutPanel(rows, currency){
  ...
  let filteredRows = selectedCat === "All" ? rows : rows.filter(it=> it.category === selectedCat);
  if(selectedBrand !== "All"){
    if(selectedBrand === "__noBrand__"){
      filteredRows = filteredRows.filter(it=> !String(it.brand||"").trim());
    } else {
      filteredRows = filteredRows.filter(it=> String(it.brand||"").trim() === selectedBrand);
    }
  }
  if(showRackFilters){
    if(filterUpright !== "All") filteredRows = filteredRows.filter(it=>{
      const p = rackPatternInfo(it); return p ? p.uprightSize === filterUpright : false;
    });
    if(filterHole !== "All") filteredRows = filteredRows.filter(it=>{
      const p = rackPatternInfo(it); return p ? p.holeSize === filterHole : false;
    });
  }
      ...
      <div class="card">
        ...
        <div class="bd" style="padding:0;">
          ...
          <div class="gymItemList">
            ...
            ${!filteredRows.length ? `<div class="muted" style="padding:16px;text-align:center;font-size:13px;">No items match the current filters.</div>` : ""}
          </div>
        </div>
      </div>
